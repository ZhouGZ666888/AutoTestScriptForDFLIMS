...
# 搜索上机批次号
seqBatchNo = (
    '.sequencingList-form-seqBatchNo input')
...
# 选择测序仪搜索框
instrument_search = '.multi-table-dialog .dialogInstrumentId-form-query input'

# 测序仪搜索按钮
instrument_search_btn = '.multi-table-dialog .baseClass-btn-search'

# 选择测序仪弹框，选择第一条
instrument_choice = (
    '.multi-table-dialog .el-dialog__body .dialog-table .vxe-table--main-wrapper .vxe-table--body-wrapper table tbody tr:nth-child(1)')
...
select_sequecing_sop_choice = (
    '.select-sopId-options .el-scrollbar__view.el-select-dropdown__list ul:nth-child(1) .el-select-group li:nth-child(1)')
...
# 待选表样本总数
all_samples = '.createTask .createTask_content_table .vxe-table--main-wrapper .vxe-table--body-wrapper table tbody tr'

# 样本lims号表单定位,根据行数
lims_number = (
    '.createTask_content_table .vxe-table--main-wrapper .vxe-table--body-wrapper table tbody tr:nth-child({}) td:nth-child(2)')

# 样本lims号表单定位，根据lims号
lims_number1 = (
    '//*[@class="createTask_content_table"]/descendant::tr/descendant::span[text()="{}"]')
...
before_concentration_adjustment_submit_comfirm = (
    '.dialog-commit-confirm .dialog-footer .baseClass-btn-confirm')
...
# 入库弹框全选按钮
storage_all_choice = (
    '.dialog-check-storage .vxe-table--main-wrapper .vxe-table--header-wrapper .vxe-checkbox--unchecked-icon')
...
after_concentration_adjustment_enter_sequencing_information = (
    '.sampleDetail_header .button-list .baseClass-btn-showDialogUseComputer')

# 浓度调整后明细表批量数据
after_concentration_adjustment_datch_data = (
    '.sampleDetail_header .button-list .baseClass-btn-batchData')

# 浓度调整后明细表批量数据,耗用量录入
after_concentration_adjustment_datch_data_used_amount = (
    '.dialogSequenceData-form-seqUsedVolumeAmt input')

# 浓度调整后明细表批量数据,测序读长
after_concentration_adjustment_datch_data_sequencer_read_length = (
    '.dialogSequenceData-form-seqReadLen input')
...
after_concentration_adjustment_confirm_sequencing = (
    '.sampleDetail_header .button-list .baseClass-btn-confirmSequening')

#浓度调整后明细表确认上机填写实际上机时间
after_concentration_adjustment_confirm_sequencing_data='.batch-edit-dialog .sequencingDetail-form-seqStartTime input'

#浓度调整后明细表确认上机填写实际上机时间确认
after_concentration_adjustment_sequencing_data_confirm='//*[@aria-label="录入实际上机时间"]/div[3]/descendant::button'



# 浓度调整后明细表生成Samplesheet按钮
after_concentration_adjustment_create_samplesheet = (
    '.sampleDetail_header .button-list .baseClass-btn-generateSampleSheet')

# 浓度调整后明细表生成Samplesheet，录入Samplesheet名称
after_concentration_adjustment_input_samplesheet_name='.sequencingDetail-form-sampleSheetName input'

# 浓度调整后明细表生成Samplesheet，录入Samplesheet名称后确认
after_concentration_adjustment_input_samplesheet_name_confirm='//*[@aria-label="请录入"]/descendant::button[3]'
...
after_concentration_adjustment_save = (
    '.createTask .baseClass-btn-saveResults')
...
after_concentration_adjustment_deposit_into_Storage = (
    '.sampleDetail_header .button-list .baseClass-btn-storage')
...
after_concentration_adjustment_submit_enter_the_result_list = (
    '.createTask .baseClass-btn-goResults')
...
result_complete_task_btn = (
    '.createTask .is-justify-end .ultrafracResult-btn-finish')
