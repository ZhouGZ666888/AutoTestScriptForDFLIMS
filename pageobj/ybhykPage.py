...
from PageElements.ybhyk_ele import *
from uitestframework.basepageTools import BasePage
from common.logs import log
from uitestframework.exceptionsTools import FileNotFound


def ybhyk_change_role(userid, role_id, vaild):
    """
    将切换用户是否为库管功能封装
    """
    ...
    sqldata = 'UPDATE sys_user_role SET is_valid= "{}"  WHERE user_id= "{}" AND role_id= "{}" '.format(vaild,userid,role_id)
...
def ybhyk_select_taskid():
    """
    将查询自己刚刚完成的样本移库单的功能封装
    """
    box_name = ybhyk_select_hw()[0]['box_name']  # 拿出之前新建的盒位信息
...
def ybyk_select_lsk_sample():
    ...
    sqldata = " SELECT t.sample_id_lims FROM sample_info_t t LEFT JOIN sample_id_lab_v tv ON (t.sample_id_lims = " \
              ...
              "sample_storage_info_t t7 ON (t6.parent_id = t7.storage_id) WHERE t.is_valid = '1' AND t.sample_status " \
    ...
    ret = executeSql.test_select_limsdb(sqldata)[0]
    return ret
...
class SampleYbyk(BasePage):
    ...
    def ybhyk_add_temporary_storage_detail(self):
        ...
        storage_name = ybhyk_select_kw()[0]['storage_name']  # 拿出之前新建的库位信息
        print(storage_name)
    ...
    def ybhyk_list_search(self):
        ...
        self.clicks('css', ybhyk_list_search_box)
        self.sleep(0.5)
    ...
    def ybyk_add_temporary_storage_detail(self):
        ...
        lsk_sample = ybyk_select_lsk_sample()['sample_id_lims']  # 拿到临时库的样本信息
        print(lsk_sample)
    ...
    def ybyk_list_search(self):
        ...
        """
        task_id = ybhyk_select_taskid()[0]['task_id']  # 拿出之前新建的盒位信息
        print(task_id)

        log.info("点击列表页，点击搜索样本移库按钮")
        self.clicks('css', ybyk_list_search_button)
        self.sleep(0.5)
...
if __name__ == '__main__':
    ...
    # test.ybyk_add_temporary_storage_detail()
    # test.ybhyk_change_role('2535','2001','0')
    ybhyk_change_role('2535', '2001', '0')