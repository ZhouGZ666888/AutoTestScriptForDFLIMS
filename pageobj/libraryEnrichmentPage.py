...
from conf.execute_sql_action import wkfj_detail_sql1, fj_next_step, wkfj_detail_sql2
...
class LibraryenrichmentPage(BasePage):
    ...
    def add_task(self):
        ...
        """
        order = get_order()
        executeSql.test_updateByParam(wkfj_detail_sql2.format(order))  # 设置预期通量
    ...
    def detail_create_result(self):
        ...
        self.sleep(1)
        result_info = self.get_text('css', result_status)
        return result_info
    ...
    def detail_into_storage(self):
        ...
        pageinfo = self.get_text('css', detail_sumbit_status)  # 入库后获取样本状态，判断是否入库成功
        print(pageinfo)
        return pageinfo
