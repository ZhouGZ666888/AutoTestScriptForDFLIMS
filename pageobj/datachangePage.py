...
from PageElements.data_modification_sjxg_ele import *
from conf.all_path import sampledata_path, dataChangFile
...
class DataChangePage(BasePage):
    ...
    def search_task(self):
        ...
        urldata = read_yaml(dataChangFile)
        ...
        self.sleep(0.5)
        # 这里调用自定义截图方法
        Screenshot(self.driver).get_img("搜索任务单号，录入来源任务单号","根据任务单号搜索出任务单")
        ...
        print('任务单号及任务单状态', search_task_id, search_task_status)
        # 返回单状态和任务单号
    ...
    def edit_task(self):
        ...
        # 这里调用自定义截图方法
        Screenshot(self.driver).get_img("进入数据修改待办tab页,点击进入按钮","进入数据修改审核页面")
        ...
        save_yaml(dataChangFile, urldata)  # 写入yaml
    ...
    def extraction_detail_datachange(self):
        ...
        self.sleep(0.5)
        self.click_by_js('css', startChange)  # 数据修改-发起修改按钮
        self.wait_loading()

        # 这里调用自定义截图方法
        Screenshot(self.driver).get_img("数据修改-点击发起修改按钮","弹出选择数据进行审核弹框")

        log.info('发起修改弹框，指定审核人下拉框')
        self.clicks('css', placeHolder_select_btn)

        log.info('发起修改弹框，选择审核人')
        self.clicks('xpath', choice_placeHolder)
        ...
        log.info('数据修改弹框-修改质检结果')
        self.clicks('css', qcResult_btn)
        self.sleep(0.5)
        self.clicks('xpath', qcResult_choice)
        self.sleep(0.5)

        log.info('数据修改弹框-完成&提交申请')
        self.clicks('css', complete_submit_btn)
        self.wait_loading()
