...
def Run_allcase():
    ...
    testResult.report(filename=str(testName) + str('_') + str(nowTime), description='用例执行情况', logPath=test_report)
...
def Auto_sendmail():
    ...
    自动发送邮件
    """
    test_report_path = report_path  # 指定存放报告的目录
    # os.system('python MyRun.py')#执行MyRun文件
    # 收件人邮箱账号,逗号隔开，发送多人邮件
    # 收件人（'guanzhou.zhou@geneseeq.com,...,xxxx@163.com'）
    receive_user = 'guoqi.dong@geneseeq.com'
    # receive_user = 'guanzhou.zhou@geneseeq.com,guanzhong.zhou@geneseeq.com,benjamin.yan@geneseeq.com'
...
def Creat_auto_shotfile():
    ...
    doc = Document()
    num = 0
    for dirName, subdirList, filename in list_dirs:
...
if __name__ == '__main__':
    ...
    result.report(filename=name, description='迪飞LIMS系统自动化-用例执行情况',
