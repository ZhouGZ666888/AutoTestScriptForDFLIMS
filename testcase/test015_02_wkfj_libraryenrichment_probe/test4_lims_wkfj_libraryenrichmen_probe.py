...
from conf.all_path import wkfj_file_path, wkfj_probe_file_path
...
class LibraryenrichMentProbe(MyTest):

    def setUp(self) -> None:
        """初始化浏览器驱动"""
        self.wkfj = LibraryenrichmentPage(self.driver)

    def test01_add_libraryenrichment_task_probe(self):
        """测试新建有探针Pooling任务类型任务单，在待选表录入任务描述，选择任务sop,检索lims号，添加、保存任务单"""
        log.info('登录系统，进入文库富集页面')
        self.initialize()
        EnterTab.enter_test_center(self.basepage)
        EnterTab.enter_pooling(self.basepage)  # 点击文库富集导航树
        log.info('登录系统，进入文库富集页面')
        self.wkfj.add_task('有探针Pooling')  # 新建任务单，选择任务类型、操作类型、sop
        log.info('核对lims，添加至任务单')
        info = self.wkfj.get_probe_sample()  # 核对lims，添加至任务单
    ...
    def test02_libraryenrichment_detail_probe(self):
        """测试有探针Pooling任务类型任务单，明细表批量粘贴导入实验数据、生成结果"""
    ...
    def test03_libraryenrichment_result_probe(self):
        """测试有探针Pooling任务类型任务单结果表批量粘贴导入实验数据、生成上机分组号，提交，完成任务单"""
        log.info("文库富集结果表，批量数据录入")
        self.wkfj.ultrasonic_result_data_input()
        log.info(' 文库富集结果表提交操作')
        pageinfo1 = self.wkfj.result_submit_sample_probe()
    ...
    def test04_detail_submit_probe(self):
        """测试有探针Pooling任务类型任务单，明细表提交、入库,完成任务单"""
        log.info("文库富集明细表提交")
    ...
    def test05_search_task_by_lims_probe(self):
        """测试有探针Pooling任务类型任务单，根据添加到的任务单中的lims样本号搜索对应的任务单"""
        EnterTab.enter_test_center(self.basepage)
        EnterTab.enter_pooling(self.basepage)  # 点击文库富集导航树

        log.info(" 测试根据添加到的任务单中的lims样本号搜索对应的任务单")
        samples = self.wkfj.serach_task(wkfj_probe_file_path)  # 根据lims号搜索任务单号
